...
Electrons interact with matter through one of two processes: elastic electron-nucleus scattering, or inelastic scattering from an atomic electron.
...
We aim to provide an estimation of $b$, the number of background events detected.
...
\end{figure}
%Refering to the Fig.~\ref{subfig:Co_setup_wall} naming system, the source was put in position $\#2$.
...
However, comparing quantitatively real data with simulations is hazardous, especially due to detector efficiency considerations, discussed in Sec.~\ref{subsec:detector_efficiency}.
Nevertheless, we can give an order of magnitude of the amount of background events detected.
%% In fact, we notice for $d^{\text{source}}<10$ events, that real data follow the same variations as for simulations, but with a slight deficit in counts.
%% That will be discussed in details in Sec.~\ref{subsec:detector_efficiency}, and is explained by
...
These results lead us to conclude that calorimeter hits, for optical modules close to the Cobalt calibration source are, for the most part, signal events.
...
%% The observable gap at the $10$ OMs limit level is due to events where the two hits occur in two optical modules each located in one half of the calorimeter, which are not represented in this histogram.


%% The discontinuity comes from the separation of the calorimeter hits in two halves.
%% In fact, the further a calorimeter block is from the source, the less Cobalt events it detects.
%% Therefore, very few events are detected coincidentally in two optical modules located far from the source.
%% %Using the data with the source in position $\#2$ and position $\#8$, we can conclude on the amount of events remaining for optical modules away from the source.
%% *Remarque : dans ce plot reste le bdf 1 gamma du cobalt dans 2 OMs*

We gave informations on background events for the whole wall, using data taken with the Cobalt source set behind the wall.
