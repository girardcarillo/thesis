...
In the course of $2019$, the commissioning of the SuperNEMO demonstrator has begun and first calorimeter data were taken.\\
...
We can also use the results to correct the time of recorded events, taking into account the time for the signal to travel through cables.
...
The theoretical linear attenuation $\alpha_{\text{att}}^{\text{th}}$, so be it the attenuation by metre of coaxial cable given in dB/m, is supplied by the constructor as
\begin{equation}
...
For the used coaxial cables, and with a frequency $f$ of few GHz, this attenuation is valued at $\alpha_{\text{att}}^{\text{th}} = 1.22$ dB/m.
In a general manner, the attenuation of a signal in dB is defined a $10$ times the decimal logarithm of a power ratio.
In the reflectometry analysis, we then define the attenuation $\mathcal{A}$, for a given length of cable $l$, as
\begin{equation}
  \mathcal{A}=10\log_{10}\frac{P_{\text{primary pulse}}}{P_{\text{secondary pulse}}} \,\text{,}
\end{equation}
where $P_{i}$ is a quantity representing the intensity of the signal.
$P$ can correspond to the maximal amplitude of the signal, as well as
the amplitude of a pulse, defined as the maximum of this pulse.
Then, we define the linear attenuation $\alpha_{\text{att}}^{R}$, measured by reflectometry and given in dB/m, with
\begin{equation}
  \mathcal{A} = \alpha_{\text{att}}^{R}l\,.
\end{equation}
The goal here is to determine the value of $\alpha_{\text{att}}^{R}$, using the reflectometry data, and to compare it with $\alpha_{\text{att}}^{\text{th}}$.
In Fig.~\ref{fig:attenuation} is shown the linear dependance between the attenuation $\mathcal{A}_{R}$ and the cable length $l$.
The fit reveals that the linear attenuation $\alpha_{\text{att}}^{R} = 0.242\pm 0.000$ dB/m, that being $5$ times smaller that expected with the value of $\alpha_{\text{att}}^{\text{th}}$.
\begin{figure}
  \centering
