\chapter{Improvement of the internal \Tl\ background rejection}
\label{ch:timediff}

At the end of September $2018$, the $34$ enriched-Selenium source foils were installed on the demonstrator.
At this time, the internal \Tl\ and \Bi\ activities of these sources had already been measured by the BiPo detector, as well as \Rn\ contamination inside the tracker.
We described in the previous chapter the impact of these activities on the final detector sensitivity to the $\zeronu$ decay, and set up optimised topological selections adjusted to reject the \Rn\ background.
...
The BiPo measurements showed that the \Tl\ contamination is about $30$ times greater than expected on average.
...
In the next section, we describe the specific features of the \Tl\ internal background.
...
For this optimisation, we use $2e$ topologies of signal simulations inside the source foils.
...
%% In that case, a simple selection allowing to discriminate signal $\zeronu$ from delayed \Tl\ event consists in rejecting $2e$ topologies for which $P_{exp}~>~$\Pint.
...
Such an optimal selection/rejection point can be found for each of the five $\sigma_{t}$ values presented.
...
For calorimeter time uncertainty better than $100$~ps the background rejection is improved despite a loss in signal selection efficiency, up to reaching $\sim45$\% of \Tl\ rejection and $\sim80$\% signal selection for an ideal calorimeter.
...
A better \Tl\ rejection can be obtained with the simple selection on time-of-flights, but the probability one has the main advantage to be more accurate as it takes into account the calorimeter time measurement uncertainties.
...
In the previous section were presented results for $\Delta t^{corr}$ and optimised probability cut-offs, and the influence of the calorimeter time resolution on these rejection techniques was reviewed.
...
The best rejection of \Tl\ is reached for the ideal calorimeter time resolution, as the two electrons time-of-flights are measured precisely.
For $\sigma_{t}=200$~ps, a smaller amount of \Tl\ background is rejected, but the $\zeronu$ selection efficiency remains stable.
This selection efficiency is affected for $\sigma_{t}=400$~ps, because the delayed events are badly discriminated compared with the simultaneous ones.
As expected, other background events ($\twonu$, \Bi\ and \Rn) are not significantly affected by this selection.
We understand the final sensitivity is degraded because the $\zeronu$ selection efficiency is reduced.
\begin{table}[h!]
  \centering
...
As discussed, the \Pint/$P_{exp}$ cut-off applied on $2e$ topologies highly degrades the final sensitivity to the $\zeronu$ process.
...
During this chapter we have defined or specified analysis tools adapted to the rejection of the delayed \Tl\ background.
